// This is the final code for my image processing work. There are additional cpp files that have some additional code for assistance in testing
...
// Struct for variables that will be passsed to flight control code.
...
int main()
{
	...
	while(-1)
    {
		...
		if (trackWindow.x == 0 || trackWindow.width > 400 || trackWindow.width > trackWindow.height || trackWindow.height > 350)
		{
...
{
	VideoCapture cap;
	// If system has multiple cameras, change integer value below to toggle cameras accessed
	cap.open(0);
...
{
	// Constants determined from camera calibration to eliminate fisheye distortion. Found in output XML file from camera calibration
...
// Function to calculate distance, velocity, and direction of movement parameters for the object being tracked
{
	...
	{
		// Uncomment these couts if you wish to see a log of important parameters for testing purposes
		...
		// This checks to only display values when the bounding box of the tracked object moves. Avoids unnecessary outputs.
		previousSelectionHeight = trackWindow.height;
		// Writes distance, velocity, and height to output txt file for control code.
		WriteFile();
	}
}
...
// Function to track the object in the camera stream using Camshift
{
	...
	// Creates color space image
	cvtColor(image, hsv, COLOR_BGR2HSV);

	if (trackObject)
	{
		int _vmin = vmin, _vmax = vmax;
		// This inRange function creates a mask based off of whether or not the pixels in the region are of a similar hsv.
		// The parameters of this function can be adjusted to change how the mask is created and can filter out unwanted pixels.
		...
		if (trackObject < 0)
		{
			...
			// calculates histogram necessary for binning pixels into separate color ranges
			calcHist(&roi, 1, 0, maskroi, hist, 1, &hsize, &phranges);
			normalize(hist, hist, 0, 255, NORM_MINMAX);

			// Starts the camshift tracking at the location that the template matched matched the input image to the source image
			trackWindow = selection;
			trackObject = 1;

			// Begins binning pixels into separate color ranges in histogram
		...
		// Backprojection is the mask that filters out pixels that do not match the hsv of the object that you wish to track.
		// Bright white is a good match, black is the thresholded mask of filtered out pixels.
		calcBackProject(&hue, 1, 0, hist, backproj, &phranges);
		backproj &= mask;
		// Applies camshift and creates a bounding box that can move and change orientation
		RotatedRect trackBox = CamShift(backproj, trackWindow,
			TermCriteria(TermCriteria::EPS | TermCriteria::COUNT, 10, 1));
		// Creates trackWindow bounding box for camshift tracking region
...
// Code to match the object captured in the uploaded image to the object located in the camera stream
{
	...
	// Read in input image. This input image will be the user inputted image of what object they want to track.
	templ = imread("/home/root/Test.jpg");
	...
	// "Method: \n 0: SQDIFF \n 1: SQDIFF NORMED \n 2: TM CCORR \n 3: TM CCORR NORMED \n 4: TM COEFF \n 5: TM COEFF NORMED";
	// Method used now is the default SQDIFF. Other methods work better under certain situations/lighting conditions.
	// I recommend playing around with this to help the inital template matching.
	int match_method = 0;
...
{
	ofstream outputFile;
	string Path = "/home/root/ParameterOutput.txt";
	// Checks if file exists. If it does, open file. If it doesn't, create file
	if (outputFile.good())
	{
		// Uncomment to open file and appends to it, keeping a running log of all values calculated
	...
	{
		// Outputs distance in meters, velocity in m/s, and height in meters. Output file is space delimited.
		outputFile << parameters.distance / 1000 << " " << velocity << " " << parameters.height / 1000 << "\n";
		outputFile.close();
	}
